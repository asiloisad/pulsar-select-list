...
class SelectListView {
  ...
  renderHelpMessage() {
    if (this.props.helpMessage && this.showHelp) {
  ...
  isHelpMode() {
    return this.props.helpMessage && this.showHelp
  }

  toggleHelp() {
    if (!this.props.helpMessage) return
  ...
  fuzzyFilter(items, query) {
    ...
    for (const item of items) {
      ...
      let score = atom.ui.fuzzyMatcher.score(string, query)
      if (modifyScore) {
