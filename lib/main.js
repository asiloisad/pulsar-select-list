...
class SelectListView {
  ...
  constructor(props) {
    ...
    const didMouseDown = (event) => {
      if (event.target === this.refs.items) {
        this.didClickItemsList = true
      }
    }
    this.element.addEventListener('mousedown', didMouseDown)
    this.disposables.add(new Disposable(() => {
      editorElement.removeEventListener('blur', didLoseFocus)
      this.element.removeEventListener('mousedown', didMouseDown)
    }))
  }
  ...
  update(props) {
    ...
    if ('preserveLastSearch' in props) {
      this.props.preserveLastSearch = props.preserveLastSearch
    }
  ...
  fuzzyFilter(items, query) {
    if (query.length === 0) {
      ...
      const replaceDiacritics = this.props.replaceDiacritics
      ...
      for (const item of items) {
        ...
        const score = atom.ui.fuzzyMatcher.score(string, query)
        if (score > 0) {
